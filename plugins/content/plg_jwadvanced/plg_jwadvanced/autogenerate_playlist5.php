...
* @subpackage autogenerate_playlist.5php
* @author joomlarulez.
...
// Directory to scan for files according to Joomla version
if(!(empty($_GET['jv']))) {
	$jversion = $_GET['jv'];
	if ($jversion != "1.5")	{
		$directory = "../../../../".$_GET['dir'];
...
foreach($items as $value) {
	...
	//assigne provider, this for html5
	if ((in_array($value2.$filterflv, $items)) || (in_array($value2.$filtermp4, $items)) || (in_array($value2.$filterm4v, $items)) || (in_array($value2.$filterm4a, $items)) || (in_array($value2.$filtermov, $items))) {
	    $value7 = "video";
	} else if (in_array($value2.$filtermp3, $items)) {
	    $value7 = "sound";
	} else if ((in_array($value2.$filterjpg, $items)) || (in_array($value2.$filterpng, $items)) || (in_array($value2.$filtergif, $items))) {
	    $value7 = "image";
	} else {
		$value7 = "";
	}
	
	//Search if there is audio/video file with playlist.image associate
	if (in_array($value2.".playlist.image".$filterjpg, $items))	{
	    $value5 = $url.$value2.".playlist.image".$filterjpg;
	} else if (in_array($value2.".playlist.image".$filterpng, $items)) {
	    $value5 = $url.$value2.".playlist.image".$filterpng;
	} else if (in_array($value2.".playlist.image".$filtergif, $items)) {
	    $value5 = $url.$value2.".playlist.image".$filtergif;
	...
	//Search if there is video file with hd video file associate
	if (in_array($value2.".hd.file".$filterflv, $items)) {
	    $value6 = $url.$value2.".hd.file".$filterflv;
	} else if (in_array($value2.".hd.file".$filtermp4, $items)) {
	    $value6 = $url.$value2.".hd.file".$filtermp4;
	} else if (in_array($value2.".hd.file".$filterm4v, $items))	{
	    $value6 = $url.$value2.".hd.file".$filterm4v;
	} else if (in_array($value2.".hd.file".$filterm4a, $items))	{
	    $value6 = $url.$value2.".hd.file".$filterm4a;
	} else if (in_array($value2.".hd.file".$filtermov, $items))	{
	    $value6 = $url.$value2.".hd.file".$filtermov;
	} else {
		$value6 = "";
	}
	...
	if ($_GET['tyt'] == "filter") {
		...
		if ($_GET['tdes'] == "description") {
			$description = $description1;
		} else if ($_GET['tdes'] == "author,description") {
			$description = $author." ".$description1;
		} else if ($_GET['tdes'] == "author,title,description") {
	...
	if (!((strpos(strtolower($value), $filterjpg) || strpos(strtolower($value), $filterpng) || strpos(strtolower($value), $filtergif)) && (in_array($value2.$filtermp3, $items) || in_array($value2.$filtermp4, $items) || in_array($value2.$filtermov, $items) || in_array($value2.$filterflv, $items) || in_array($value2.$filterm4a, $items) || in_array($value2.$filterm4v, $items)))) {
		if (!((strpos(strtolower($value), $filterjpg) && ($_GET['jpg'] == 'false')) || (strpos(strtolower($value), $filterpng) && ($_GET['png'] == 'false')) || (strpos(strtolower($value), $filtergif) && ($_GET['gif'] == 'false')))) {
			//search if there is caption file or a playlist.image or a HD file associate with audio/video file and not display if not
			if ((!($value4 == $url.$value)) && (!(strpos(strtolower($value), "playlist.image"))) && (!(strpos(strtolower($value), "hd.file")))) {
				echo "\n\t<item>\n";
				if (!(empty($title))) {
				...
				if (!(empty($value5))) {
					echo "\t\t<jwplayer:playlist.image>".$value5."</jwplayer:playlist.image>\n";
				}
				if (!(empty($value6))) {
					echo "\t\t<jwplayer:hd.file>".$value6."</jwplayer:hd.file>\n";
				}
				if (!(empty($value7))) {
					echo "\t\t<jwplayer:provider>".$value7."</jwplayer:provider>\n";
				}
				if (!(empty($link))) {
					echo "\t\t<link>".$link."</link>\n";
				}
