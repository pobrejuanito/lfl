...
if(in_array('sharing-2', $is_plgplayer_plugin_array)) {
	$is_plgplayer_sharing_enabled = '1';
	$key = array_search( 'sharing-2' , $is_plgplayer_plugin_array);
	unset($is_plgplayer_plugin_array[$key]);
	//set version of the plugin
	$is_plgplayer_sharing_version = "sharing-2";
} else {
	$is_plgplayer_sharing_enabled = $params->get('SharingPluginEnabled', "0");
	//set version of the plugin
	$is_plgplayer_sharing_version = $params->get('SharingPluginversion', "2");
	$is_plgplayer_sharing_version = "sharing-".$is_plgplayer_sharing_version;
}
if($is_plgplayer_sharing_enabled == '1' && $is_plgplayer_sharing_version == 'sharing-2') {
