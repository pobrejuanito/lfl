...
<form id="searchbox" action="<?php echo JURI::base() ?>index.php?option=com_search" method="post" role="search">
...
<script>
jQuery(function($) {
	$('#searchbox input[name=searchword]').search({'url': '<?php /* echo JRoute::_("index.php?option=com_search&tmpl=raw&type=json&ordering=&searchphrase=all");*/ echo JURI::base() . 'index.php?option=com_search&tmpl=raw&type=json&ordering=&searchphrase=all'; ?>', 'param': 'searchword', 'msgResultsHeader': '<?php echo JText::_("TPL_WARP_SEARCH_RESULTS"); ?>', 'msgMoreResults': '<?php echo JText::_("TPL_WARP_SEARCH_MORE"); ?>', 'msgNoResults': '<?php echo JText::_("TPL_WARP_SEARCH_NO_RESULTS"); ?>'}).placeholder();
});
</script>
