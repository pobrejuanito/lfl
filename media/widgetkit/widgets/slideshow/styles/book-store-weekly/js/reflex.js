		...
		if(canvas.getContext && image.width>=32 && image.height>=32) {
			...
			if(tilt=='l'||tilt=='r') {
				if(resource.getContext) {
					...
					if(tilt=='r') {
						for(j=0;j<iter;j++) {
							context.drawImage(canvas,j*wide,0,wide,resource.height,j*wide,j*1,wide,resource.height-(j*3));
						}
						if(rest>0) {
							rest = canvas.width-(iter*wide);
							context.drawImage(canvas,j*wide,0,rest,resource.height,j*wide,j*1,rest,resource.height-(j*3));
						}
					}else {
						for(j=0;j<iter;j++) {
							context.drawImage(canvas,j*wide,0,wide,resource.height,j*wide,(iter-j)*1,wide,resource.height-((iter-j)*3));
						}
						if(rest>0) {
							rest = canvas.width-(iter*wide);
