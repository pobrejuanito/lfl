...
class ArisI18N
{
	...
	function ArisI18N($resourceDir, $locale, $cacheDir = null, $cacheGroup = null, $defaultLocale = 'en', $tagMapping = null)
	{
		...
		$this->_cache = $cacheDir != null 
			? new ArisI18NCache($locale, $resourceDir, $cacheDir, $cacheGroup, $tagMapping) 
			: null; 
	}
...
class ArisI18NHelper
{
	...
	function parseXml($xml, &$tagMapping)
	{
		...
		if (!empty($xml))
		{
			...
			$itemTagName = $tagMapping->ItemTagName;
			$itemIdAttr = $tagMapping->ItemIdAttrName;
			//echo isset($xml->document->$itemTagName) ? 'yes' : 'no';exit(); 
		...
		else
		{
			...
			if (file_exists($resFile))
			{
				if (file_exists($cachedFile))
				{
					...
					// if cached file modified date greater than modified date res file
